...
for (depvar in depvar_array) {
        ...
        write_rds(slim_fixest(results, depvar),  # this speeds up the process
                  sprintf("output/results_%s_%s_highcost_year.rds", depvar, 
...
listregs <- lapply(seq_len(nrow(specs)), 
        function(k) {
        read_rds(sprintf("output/results_%s_%s_highcost_year.rds", 
                                   specs$depvar[k], 
                                   gsub(".", "_", 
...
listregs %>% etable(keep = c("%below_limit:treated:time_0", 
        ...
        write_lines("tables/impact_conforming_market_highcost_year.tex")
...
listregs %>% etable(keep = c("%below_limit:treated:time_0", 
                ...
                extralines = list("Post Hurricane joint test" = teststats, 
                                  "Post Hurricane p-value" = pvalues)) %>%
        write_lines("tables/impact_conforming_market_narrower_highcost_year.tex")
