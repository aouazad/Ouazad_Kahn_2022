...
for (depvar in depvar_array) {
        ...
        regf <- as.formula(sprintf(paste0("%s ~ %s + %s + %s + %s + %s + %s + %s + %s", # covariates
                                          "+ below_limit + below_limit:treated + below_limit:as.factor(name_event) + below_limit:treated:as.factor(name_event) + below_limit:highcost + below_limit:highcost:treated ",
                                          "| as.factor(agency) + as.factor(highcost) + as.factor(year) + as.factor(ZCTA5CE10) + as.factor(name_event)" # fixed effects
                                          ), 
                                   depvar,
                                   ...
                                   paste(sprintf("below_limit:year_%d",
                                         ...
                                         collapse = " + "),
                                   paste(sprintf("year_%d:highcost",
                                                 (abs(range(est_sample_filtered$year)[1]):2012)),
                                         collapse = " + "),
                                   paste(sprintf("year_%d:highcost",
                                                 2014:range(est_sample_filtered$year)[2]),
        ...
        results <- feols(regf, 
                         data = est_sample_filtered %>% 
                                filter(year >= 2001), # this year filter speeds up estimation, but doesn't affect the estimation
        ...
        write_rds(slim_fixest(results, depvar),  # this speeds up the process
                  sprintf("output/results_%s_%s_agency_highcost_year.rds", depvar, 
...
listregs <- lapply(seq_len(nrow(specs)), 
        function(k) {
        read_rds(sprintf("output/results_%s_%s_agency_highcost_year.rds", 
                                   specs$depvar[k], 
                                   gsub(".", "_", 
...
listregs %>% etable(keep = c("%below_limit:treated:time_0", 
        ...
        write_lines("tables/impact_conforming_market_agency_highcost_year.tex")
...
listregs %>% etable(keep = c("%below_limit:treated:time_0", 
                ...
                extralines = list("Post Hurricane joint test" = teststats, 
                                  "Post Hurricane p-value" = pvalues)) %>%
        write_lines("tables/impact_conforming_market_narrower_agency_highcost_year.tex")
