...
for (depvar in depvar_array) {
        ...
        regf <- as.formula(sprintf(paste0("%s ~ %s + %s + %s + %s ", # covariates
                                          "+ below_limit + below_limit:treated ",
                                          "| as.factor(year) + as.factor(ZCTA5CE10) " # fixed effects
                                          ), 
                                   depvar,
        ...
        write_rds(slim_fixest(results, depvar),  # this speeds up the process
                  sprintf("output/results_%s_%s_2wfe.rds", depvar, 
...
listregs <- lapply(seq_len(nrow(specs)), 
        function(k) {
        read_rds(sprintf("output/results_%s_%s_2wfe.rds", 
                                   specs$depvar[k], 
                                   gsub(".", "_", 
...
listregs %>% etable(keep = c("%below_limit:treated:time_0", 
        ...
        write_lines("tables/impact_conforming_market_2wfe.tex")
...
listregs %>% etable(keep = c("%below_limit:treated:time_0", 
                ...
                extralines = list("Post Hurricane joint test" = teststats, 
                                  "Post Hurricane p-value" = pvalues)) %>%
        write_lines("tables/impact_conforming_market_narrower_2wfe.tex")
