...
for (depvar in depvar_array) {
        ...
        regf <- as.formula(sprintf(paste0("%s ~ %s + %s + %s + %s + %s + %s ", # covariates
                                          ...
                                          "| as.factor(highcost) + as.factor(agency) + as.factor(year) + as.factor(ZCTA5CE10) + as.factor(name_event)" # fixed effects
                                          ), 
                                   depvar,
        ...
        pdf(paste0("figures/impact_on_jumbo_conforming_market_", 
                  depvar, "_agency_and_highcost.pdf"))   
        ...
        write_rds(slim_fixest(results, depvar),  # this speeds up the process
                  sprintf("output/results_%s_%s_agency_and_highcost.rds", depvar, 
...
listregs <- lapply(seq_len(nrow(specs)), 
        function(k) {
        read_rds(sprintf("output/results_%s_%s_agency_and_highcost.rds", 
                                   specs$depvar[k], 
                                   gsub(".", "_", 
...
listregs %>% etable(keep = c("%below_limit:treated:time_0", 
        ...
        write_lines("tables/impact_conforming_market_agency_and_highcost.tex")
...
listregs %>% etable(keep = c("%below_limit:treated:time_0", 
                ...
                extralines = list("Post Hurricane joint test" = teststats, 
                                  "Post Hurricane p-value" = pvalues)) %>%
        write_lines("tables/impact_conforming_market_narrower_agency_and_highcost.tex")
