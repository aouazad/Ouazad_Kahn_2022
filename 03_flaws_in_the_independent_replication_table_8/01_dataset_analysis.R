...
# non mutually exclusive time dummies

originated_05_treatment <- originated_05 %>%
  filter(Treatment == 1) %>%
  mutate(sum =
...
tibble(
  ...
  `Sum is 0` = sum(originated_05_treatment$sum==0),
  `Sum is 1` = sum(originated_05_treatment$sum==1),
  `Sum is 2` = sum(originated_05_treatment$sum==2),
  `Sum is 3` = sum(originated_05_treatment$sum==3)) %>%
  xtable(digits=0,
         align = c("l", "l", "l", "c", "c", "c"))  %>%
  print(include.rownames = FALSE, floating = FALSE) %>%
  write_lines("tables/non_mutually_exclusive_time_dummies.tex")
...
xtable(digits=0,
  ...
  print(include.rownames = FALSE,
        floating = FALSE) %>%
  write_lines("tables/incorrect_treatment_years.tex")
